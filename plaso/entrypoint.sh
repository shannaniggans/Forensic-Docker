...
log2timeline.py --artifact-filters NTFSMFTFiles --parser mft --partitions all --no_vss --workers ${WORKER_NUM:-2} --buffer_size 2048M --profiling_sample_rate 10000 -d --log-file=/data/test/log2timeline.log /data/test/output-mft.plaso /data/*flat.vmdk &&
...
(echo "["; cat /data/test/output-mft.json; echo "]") > /data/test/array-mft.json && 
/bin/sed -i ':a;N;$!ba;s/,]/]/g' /data/test/array-mft.json &&
...
(echo "["; cat /data/test/output-winreg.json; echo "]") > /data/test/array-winreg.json && 
/bin/sed -i ':a;N;$!ba;s/,]/]/g' /data/test/array-winreg.json &&
...
(echo "["; cat /data/test/output-win7.json; echo "]") > /data/test/array-win7.json && 
/bin/sed -i ':a;N;$!ba;s/,]/]/g' /data/test/array-win7.json &&
...
(echo "["; cat /data/test/output-webhist.json; echo "]") > /data/test/array-webhist.json && 
/bin/sed -i ':a;N;$!ba;s/,]/]/g' /data/test/array-webhist.json &&
...
(echo "["; cat /data/test/output-win_gen.json; echo "]") > /data/test/array-win_gen.json &&
/bin/sed -i ':a;N;$!ba;s/,]/]/g' /data/test/array-win_gen.json &&
...
(echo "["; cat /data/test/output-evtx.json; echo "]") > /data/test/array-evtx.json && 
/bin/sed -i ':a;N;$!ba;s/,]/]/g' /data/test/array-evtx.json